...
public class TPluginResolver {
    ...
    public static void configure(TPluginScanner scanner) {
        ...
        for (Class<? extends TPlugin> pluginType : scanner.scanPluginTypes()) {
            String typeName = PluginTools.typeName(pluginType); // typeName: input, output etc.
            Map<String, Entry> cmap = pluginClassMap.computeIfAbsent(typeName, k -> new HashMap<>());
            for (Class<? extends TPlugin> pc : scanner.scanPluginClasses(pluginType)) {
                PluginName pluginName = PluginTools.pluginName(pc);
    ...
    public <T extends TPlugin> T resolve(Class<? extends T> type, String pluginName, String minorName) throws TPluginNotFoundException {
        return (T)resolve(PluginTools.typeName(type), pluginName, minorName);
    }

    public TPlugin resolve(String typeName, String pluginName, String minorName) throws TPluginNotFoundException{
